...
const render = (employees) => {
  ...
  num = Math.floor(html.length / 3);

  //When html.length is NOT evenly divided into 3
  if (html.length / 3 === num) {
    for (let i = 0; i < num - 1; i++) {
      html.splice((i + 1) * 3 + i, 0, newRow);
    }
  } else {
    //When html.length is evenly divided into 3, we don't want to add
    //the last newRow code
    for (let i = 0; i < num; i++) {
      html.splice((i + 1) * 3 + i, 0, newRow);
    }
  }
